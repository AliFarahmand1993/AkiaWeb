...
ALTER TABLE wp_rankie_ranks	
  ...
  PRIMARY KEY (`id`)
)   DEFAULT CHARSET=utf8 AUTO_INCREMENT=161 ;
...
)   DEFAULT CHARSET=utf8 AUTO_INCREMENT=368 ;
...
CREATE TABLE IF NOT EXISTS `{$prefix}rankie_log` (
  ...
  `date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
)   DEFAULT CHARSET=utf8 AUTO_INCREMENT=55 ;
...
CREATE TABLE IF NOT EXISTS `{$prefix}rankie_ranks` (
  ...
  PRIMARY KEY (`id`)
)   DEFAULT CHARSET=utf8 AUTO_INCREMENT=536 ;
	";
		// executing quiries
		$que = explode ( ';', $querys );
		foreach ( $que as $query ) {
