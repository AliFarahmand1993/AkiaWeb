	...
	class WP_Package_Updater {
		...
		protected function do_query_license( $query_type ) {
			...
			if ( JSON_ERROR_NONE !== json_last_error() ) {
				$license_data          = new stdClass();
				$license_data->message = __( 'لطفا کلید لایسنس خود را برسی نمایید', 'wp-package-updater' );
		...
		protected function handle_license_errors( $license_data, $query_type = null ) {
			...
			if ( 'activate' === $query_type ) {
				...
				if ( isset( $license_data->allowed_domains ) ) {
					$license_data->message = __( 'این لایسنس برای این دامنه در حال استفاده می باشد', 'wp-package-updater' );
				} elseif ( isset( $license_data->max_allowed_domains ) ) {
					$license_data->clear_key = true;
					$license_data->message   = __( 'تعداد دفعات استفاده مجاز از لایسنس به پایان رسیده و برای این دامنه قابل استفاده نمی باشد', 'wp-package-updater' );
				}
			} elseif ( 'deactivate' === $query_type ) {

				if ( isset( $license_data->allowed_domains ) ) {
					$license_data->clear_key = true;
					$license_data->message   = __( 'لایسنس برای این دامنه غیر فعال می باشد', 'wp-package-updater' );
				}
			}
			...
			if (
				...
				if ( isset( $license_data->date_expiry ) ) {
					...
					$license_data->message = __( 'لایسنس منقضی می باشد', 'wp-package-updater' );
				}
			} elseif (
				isset( $license_data->status ) &&
				'blocked' === $license_data->status
			) {
				$license_data->message = __( 'لطفا از یک لایسنس دیگر استفاده نمایید', 'wp-package-updater' );
			} elseif (
				isset( $license_data->status ) &&
				'pending' === $license_data->status
			) {
				$license_data->clear_key = true;
				$license_data->message   = __( 'در حال حاظر لایسنس در انتظار تایید می باشد. لطفا مجدد امتحان کنید', 'wp-package-updater' );
			} elseif (
				isset( $license_data->status ) &&
				'invalid' === $license_data->status
			) {
				$license_data->clear_key = true;
				$license_data->message   = __( 'لطفا از یک لایسنس معتبر استفاده کنید', 'wp-package-updater' );
			} elseif ( isset( $license_data->license_key ) ) {
				$license_data->clear_key = true;
				$license_data->message   = __( 'لطفا از یک لایسنس معتبر استفاده نمایید', 'wp-package-updater' );
			} elseif ( 1 === count( (array) $license_data ) ) {

				if ( 'Plugin' === $this->type ) {
					$license_data->message = __( 'برای بروزرسانی افزونه به یک لایسنس معتبر نیاز دارید', 'wp-package-updater' );
				} else {
				...
				if ( 'Plugin' === $this->type ) {
					$license_data->message = __( 'یک خطای غیر منتظره رخ داده است، لطفا دوباره امتحان کنید. در صورتی که مشکل همچنان ادامه دارد لطفا از طریق تیکت با ما در ارتباط باشید', 'wp-package-updater' );
				} else {
